...
function write_generated_defs(generated_defs_dir::String, open62541_header_dir::String)
    type_s = """
    ...
    types_ambiguous_blacklist = [:UA_Duration, :UA_ByteString, :UA_XmlElement, :UA_LocaleId, :UA_DateTime, :UA_UtcTime, :UA_StatusCode] 

    """
    
    inlined_s = """
    # Vector of all inlined function names listed in the amalgamated open62541 header file
    inlined_funcs = $(extract_inlined_funcs(open62541_header_dir))
    """
...
function extract_inlined_funcs(open62541_header_dir::String)
    regex_inlined = r"UA_INLINE[\s]+(?:[\w\*]+[\s]*[\s\S]){0,3}((?:__)?UA_[\w]+)\("
    inlined_funcs = String[]
    open(open62541_header_dir, "r") do f
        data = read(f, String)
...
# Extract all inlined functions and move them to codegen blacklist
append!(options["general"]["printer_blacklist"], extract_inlined_funcs(open62541_header))
