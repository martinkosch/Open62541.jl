...
In this tutorial, we will showcase how basic access control can be used to restrict 
clients connecting to a server in what they are allowed to do on the server. 
Specifically, we will configure that a client is allowed to add nodes and 
references, as well as delete references. However, we will disallow the client 
to delete nodes from the server.
